module.exports = {
  en: {
    default: true,
    path: "en",
    locale: "English",
    homeTitle: "Home",
    siteTitle: "Saeed Esmaili, Data Nerd",
    blogAuthor: {
      name: "Saeed Esmaili",
      summary:
        ", a data professional. I enjoy playing with, analyzing, and talking about any type of data.",
    },
    writtenBy: "Written by",
    otherBlog: "persian posts",
    email: "email",
    twitter: "twitter",
    github: "github",
    linkedin: "linkedin",
    rss: "rss",
    minsRead: "mins read",
    description:
      "Written by Saeed Esmaili, a data professional. I enjoy playing with, analyzing, and talking about any type of data.",
    translation: "persian",
  },
  fa: {
    path: "fa",
    locale: "Farsi",
    homeTitle: "خانه",
    siteTitle: "دیتانرد، سعید اسماعیلی",
    blogAuthor: {
      name: "سعید اسماعیلی",
      summary:
        "، متخصص تحلیل دیتا، و علاقمند به کنکاش، بررسی و صحبت درباره هر نوع دیتا.",
    },
    writtenBy: "نوشته‌های",
    otherBlog: "نوشته‌های انگلیسی",
    email: "ایمیل",
    twitter: "توییتر",
    github: "گیت‌هاب",
    linkedin: "لینکدین",
    rss: "فید",
    minsRead: "دقیقه",
    description:
      "نوشته‌های سعید اسماعیلی، متخصص تحلیل دیتا، و علاقمند به کنکاش، بررسی و صحبت درباره هر نوع دیتا.",
    translation: "انگلیسی",
  },
}
